...
// Generated enums and structures for device STM32G431xx version 1.9
...
struct FDCAN_Type {
	...
	__IO uint8_t ILS; // @88 The Interrupt Line Select register assigns an interrupt generated by a specific interrupt flag from the
	                  // Interrupt Register to one of the two module interrupt lines. For interrupt generation the respective interrupt line
	                  // has to be enabled via ILE[EINT0] and ILE[EINT1].
	...
	__IO uint32_t RXGFC;   // @128 Global settings for Message ID filtering. The Global Filter Configuration controls the filter path for
	                       // standard and extended messages as described in Figure706: Standard Message ID filter path and Figure707:
...
inline void fdcan_nbtp_set_ntseg2(struct FDCAN_Type* p, uint32_t val) {
	p->NBTP = (p->NBTP & ~FDCAN_NBTP_NTSEG2) | ((val << 0) & FDCAN_NBTP_NTSEG2);
}
...
inline uint32_t fdcan_nbtp_get_ntseg2(struct FDCAN_Type* p) { return (p->NBTP & FDCAN_NBTP_NTSEG2) >> 0; }
...
inline uint32_t fdcan_ecr_get_rec(struct FDCAN_Type* p) { return (p->ECR & FDCAN_ECR_REC) >> 8; }
...
inline void fdcan_rxgfc_set_lse(struct FDCAN_Type* p, uint32_t val) {
	p->RXGFC = (p->RXGFC & ~FDCAN_RXGFC_LSE) | ((val << 24) & FDCAN_RXGFC_LSE);
}
inline void fdcan_rxgfc_set_lss(struct FDCAN_Type* p, uint32_t val) {
	p->RXGFC = (p->RXGFC & ~FDCAN_RXGFC_LSS) | ((val << 16) & FDCAN_RXGFC_LSS);
}
...
inline uint32_t fdcan_rxgfc_get_lse(struct FDCAN_Type* p) { return (p->RXGFC & FDCAN_RXGFC_LSE) >> 24; }
inline uint32_t fdcan_rxgfc_get_lss(struct FDCAN_Type* p) { return (p->RXGFC & FDCAN_RXGFC_LSS) >> 16; }
...
inline uint32_t fdcan_txbrp_get_trp(struct FDCAN_Type* p) { return (p->TXBRP & FDCAN_TXBRP_TRP) >> 0; }

// FDCAN->TXBAR FDCAN Tx Buffer Add Request Register
enum {
	FDCAN_TXBAR_AR = ((1UL << 3) - 1) << 0, // AR
};
inline void fdcan_txbar_set_ar(struct FDCAN_Type* p, uint32_t val) {
	p->TXBAR = (p->TXBAR & ~FDCAN_TXBAR_AR) | ((val << 0) & FDCAN_TXBAR_AR);
}
inline uint32_t fdcan_txbar_get_ar(struct FDCAN_Type* p) { return (p->TXBAR & FDCAN_TXBAR_AR) >> 0; }

// FDCAN->TXBCR FDCAN Tx Buffer Cancellation Request Register
enum {
	FDCAN_TXBCR_CR = ((1UL << 3) - 1) << 0, // CR
};
inline void fdcan_txbcr_set_cr(struct FDCAN_Type* p, uint32_t val) {
	p->TXBCR = (p->TXBCR & ~FDCAN_TXBCR_CR) | ((val << 0) & FDCAN_TXBCR_CR);
}
inline uint32_t fdcan_txbcr_get_cr(struct FDCAN_Type* p) { return (p->TXBCR & FDCAN_TXBCR_CR) >> 0; }

// FDCAN->TXBTO FDCAN Tx Buffer Transmission Occurred Register
enum {
	FDCAN_TXBTO_TO = ((1UL << 3) - 1) << 0, // TO
};
inline uint32_t fdcan_txbto_get_to(struct FDCAN_Type* p) { return (p->TXBTO & FDCAN_TXBTO_TO) >> 0; }

// FDCAN->TXBCF FDCAN Tx Buffer Cancellation Finished Register
enum {
	FDCAN_TXBCF_CF = ((1UL << 3) - 1) << 0, // CF
};
inline uint32_t fdcan_txbcf_get_cf(struct FDCAN_Type* p) { return (p->TXBCF & FDCAN_TXBCF_CF) >> 0; }

// FDCAN->TXBTIE FDCAN Tx Buffer Transmission Interrupt Enable Register
enum {
	FDCAN_TXBTIE_TIE = ((1UL << 3) - 1) << 0, // TIE
};
inline void fdcan_txbtie_set_tie(struct FDCAN_Type* p, uint32_t val) {
	p->TXBTIE = (p->TXBTIE & ~FDCAN_TXBTIE_TIE) | ((val << 0) & FDCAN_TXBTIE_TIE);
}
inline uint32_t fdcan_txbtie_get_tie(struct FDCAN_Type* p) { return (p->TXBTIE & FDCAN_TXBTIE_TIE) >> 0; }

// FDCAN->TXBCIE FDCAN Tx Buffer Cancellation Finished Interrupt Enable Register
enum {
	FDCAN_TXBCIE_CFIE = ((1UL << 3) - 1) << 0, // CFIE
};
inline void fdcan_txbcie_set_cfie(struct FDCAN_Type* p, uint32_t val) {
	p->TXBCIE = (p->TXBCIE & ~FDCAN_TXBCIE_CFIE) | ((val << 0) & FDCAN_TXBCIE_CFIE);
}
inline uint32_t fdcan_txbcie_get_cfie(struct FDCAN_Type* p) { return (p->TXBCIE & FDCAN_TXBCIE_CFIE) >> 0; }
...
enum {
	...
	PWR_CR1_DBP = 1UL << 8, // Disable backup domain write protection In reset state, the RTC and backup registers are protected against
	                        // parasitic write access. This bit must be set to enable write access to these registers.
	PWR_CR1_FPD_STOP = 1UL << 3, // FPD_STOP
	PWR_CR1_LPMS =
	    ((1UL << 3) - 1)
	    << 0, // Low-power mode selection These bits select the low-power mode entered when CPU enters the deepsleep mode. 1xx: Shutdown
	          // mode Note: In Standby mode, SRAM2 can be preserved or not, depending on RRS bit configuration in PWR_CR3.
};
...
enum {
	...
	PWR_CR2_PVDLS =
	    ((1UL << 3) - 1)
	    << 1, // Programmable voltage detector level selection. These bits select the PVD falling threshold: Note: These bits are
	          // write-protected when the PVDL bit is set in the SYSCFG_CFGR2 register. The protection can be reset only by a system reset.
	PWR_CR2_PVDE = 1UL << 0, // Programmable voltage detector enable Note: This bit is write-protected when the PVDL bit is set in the
...
enum {
	...
	PWR_CR3_UCPD1_DBDIS =
	    1UL
	    << 14, // USB Type-C and Power Delivery Dead Battery disable. After exiting reset, the USB Type-C “dead battery” behavior is
	           // enabled, which may have a pull-down effect on CC1 and CC2 pins. It is recommended to disable it in all cases, either to
	           // stop this pull-down or to hand over control to the UCPD1 (which should therefore be initialized before doing the disable).
	PWR_CR3_UCPD1_STDBY = 1UL << 13, // UCPD1_STDBY USB Type-C and Power Delivery standby mode.
	PWR_CR3_APC = 1UL << 10, // Apply pull-up and pull-down configuration When this bit is set, the I/O pull-up and pull-down configurations
	                         // defined in the PWR_PUCRx and PWR_PDCRx registers are applied. When this bit is cleared, the PWR_PUCRx and
	                         // PWR_PDCRx registers are not applied to the I/Os.
	PWR_CR3_RRS   = 1UL << 8, // SRAM2 retention in Standby mode
	PWR_CR3_EWUP5 = 1UL << 4, // Enable Wakeup pin WKUP5 When this bit is set, the external wakeup pin WKUP5 is enabled and triggers a
	                          // wakeup from Standby or Shutdown event when a rising or a falling edge occurs.The active edge is configured
	                          // via the WP5 bit in the PWR_CR4 register.
	PWR_CR3_EWUP4 = 1UL << 3, // Enable Wakeup pin WKUP4 When this bit is set, the external wakeup pin WKUP4 is enabled and triggers a
	                          // wakeup from Standby or Shutdown event when a rising or a falling edge occurs. The active edge is configured
	                          // via the WP4 bit in the PWR_CR4 register.
	PWR_CR3_EWUP3 = 1UL << 2, // Enable Wakeup pin WKUP3 When this bit is set, the external wakeup pin WKUP3 is enabled and triggers a
	                          // wakeup from Standby or Shutdown event when a rising or a falling edge occurs. The active edge is configured
	                          // via the WP3 bit in the PWR_CR4 register.
	PWR_CR3_EWUP2 = 1UL << 1, // Enable Wakeup pin WKUP2 When this bit is set, the external wakeup pin WKUP2 is enabled and triggers a
	                          // wakeup from Standby or Shutdown event when a rising or a falling edge occurs. The active edge is configured
	                          // via the WP2 bit in the PWR_CR4 register.
	PWR_CR3_EWUP1 = 1UL << 0, // Enable Wakeup pin WKUP1 When this bit is set, the external wakeup pin WKUP1 is enabled and triggers a
	                          // wakeup from Standby or Shutdown event when a rising or a falling edge occurs. The active edge is configured
...
enum {
	...
	PWR_CR4_WP5 =
	    1UL << 4, // Wakeup pin WKUP5 polarity This bit defines the polarity used for an event detection on external wake-up pin, WKUP5
	PWR_CR4_WP4 =
	    1UL << 3, // Wakeup pin WKUP4 polarity This bit defines the polarity used for an event detection on external wake-up pin, WKUP4
	PWR_CR4_WP3 =
	    1UL << 2, // Wakeup pin WKUP3 polarity This bit defines the polarity used for an event detection on external wake-up pin, WKUP3
	PWR_CR4_WP2 =
	    1UL << 1, // Wakeup pin WKUP2 polarity This bit defines the polarity used for an event detection on external wake-up pin, WKUP2
	PWR_CR4_WP1 =
	    1UL << 0, // Wakeup pin WKUP1 polarity This bit defines the polarity used for an event detection on external wake-up pin, WKUP1
};

// PWR->SR1 Power status register 1
enum {
	PWR_SR1_WUFI = 1UL << 15, // Wakeup flag internal This bit is set when a wakeup is detected on the internal wakeup line. It is cleared
	                          // when all internal wakeup sources are cleared.
	PWR_SR1_SBF = 1UL << 8,   // Standby flag This bit is set by hardware when the device enters the Standby mode and is cleared by setting
	                          // the CSBF bit in the PWR_SCR register, or by a power-on reset. It is not cleared by the system reset.
	PWR_SR1_WUF5 = 1UL << 4, // Wakeup flag 5 This bit is set when a wakeup event is detected on wakeup pin, WKUP5. It is cleared by writing
	                         // ‘1’ in the CWUF5 bit of the PWR_SCR register.
	PWR_SR1_WUF4 = 1UL << 3, // Wakeup flag 4 This bit is set when a wakeup event is detected on wakeup pin,WKUP4. It is cleared by writing
	                         // ‘1’ in the CWUF4 bit of the PWR_SCR register.
	PWR_SR1_WUF3 = 1UL << 2, // Wakeup flag 3 This bit is set when a wakeup event is detected on wakeup pin, WKUP3. It is cleared by writing
	                         // ‘1’ in the CWUF3 bit of the PWR_SCR register.
	PWR_SR1_WUF2 = 1UL << 1, // Wakeup flag 2 This bit is set when a wakeup event is detected on wakeup pin, WKUP2. It is cleared by writing
	                         // ‘1’ in the CWUF2 bit of the PWR_SCR register.
	PWR_SR1_WUF1 = 1UL << 0, // Wakeup flag 1 This bit is set when a wakeup event is detected on wakeup pin, WKUP1. It is cleared by writing
	                         // ‘1’ in the CWUF1 bit of the PWR_SCR register.
};

// PWR->SR2 Power status register 2
enum {
	PWR_SR2_PVMO2 = 1UL << 15, // Peripheral voltage monitoring output: V<sub>DDA</sub> vs. 1.8 V Note: PVMO2 is cleared when PVM2 is
	                           // disabled (PVME = 0). After enabling PVM2, the PVM2 output is valid after the PVM2 wakeup time.
	PWR_SR2_PVMO1 = 1UL << 14, // Peripheral voltage monitoring output: V<sub>DDA</sub> vs. 1.62 V Note: PVMO1 is cleared when PVM1 is
	                           // disabled (PVME = 0). After enabling PVM1, the PVM1 output is valid after the PVM1 wakeup time.
	PWR_SR2_PVDO = 1UL << 11,  // Programmable voltage detector output
	PWR_SR2_VOSF =
	    1UL << 10, // Voltage scaling flag A delay is required for the internal regulator to be ready after the voltage scaling has been
	               // changed. VOSF indicates that the regulator reached the voltage level defined with VOS bits of the PWR_CR1 register.
	PWR_SR2_REGLPF =
	    1UL << 9, // Low-power regulator flag This bit is set by hardware when the MCU is in Low-power run mode. When the MCU exits the
	              // Low-power run mode, this bit remains at 1 until the regulator is ready in main mode. A polling on this bit must be done
	              // before increasing the product frequency. This bit is cleared by hardware when the regulator is ready.
	PWR_SR2_REGLPS = 1UL << 8, // Low-power regulator started This bit provides the information whether the low-power regulator is ready
	                           // after a power-on reset or a Standby/Shutdown. If the Standby mode is entered while REGLPS bit is still
...
enum {
	PWR_PUCRA_PU15 = 1UL << 15, // Port A pull-up bit 15 When set, this bit activates the pull-up on PA[15] when APC bit is set in PWR_CR3
	                            // register. The pull-up is not activated if the corresponding PD15 bit is also set.
	PWR_PUCRA_PU13 = 1UL << 13, // Port A pull-up bit y (y=0..13) When set, this bit activates the pull-up on PA[y] when APC bit is set in
	                            // PWR_CR3 register. The pull-up is not activated if the corresponding PDy bit is also set.
	PWR_PUCRA_PU12 = 1UL << 12, // Port A pull-up bit y (y=0..13) When set, this bit activates the pull-up on PA[y] when APC bit is set in
	                            // PWR_CR3 register. The pull-up is not activated if the corresponding PDy bit is also set.
	PWR_PUCRA_PU11 = 1UL << 11, // Port A pull-up bit y (y=0..13) When set, this bit activates the pull-up on PA[y] when APC bit is set in
	                            // PWR_CR3 register. The pull-up is not activated if the corresponding PDy bit is also set.
	PWR_PUCRA_PU10 = 1UL << 10, // Port A pull-up bit y (y=0..13) When set, this bit activates the pull-up on PA[y] when APC bit is set in
	                            // PWR_CR3 register. The pull-up is not activated if the corresponding PDy bit is also set.
	PWR_PUCRA_PU9 = 1UL << 9,   // Port A pull-up bit y (y=0..13) When set, this bit activates the pull-up on PA[y] when APC bit is set in
	                            // PWR_CR3 register. The pull-up is not activated if the corresponding PDy bit is also set.
	PWR_PUCRA_PU8 = 1UL << 8,   // Port A pull-up bit y (y=0..13) When set, this bit activates the pull-up on PA[y] when APC bit is set in
	                            // PWR_CR3 register. The pull-up is not activated if the corresponding PDy bit is also set.
	PWR_PUCRA_PU7 = 1UL << 7,   // Port A pull-up bit y (y=0..13) When set, this bit activates the pull-up on PA[y] when APC bit is set in
	                            // PWR_CR3 register. The pull-up is not activated if the corresponding PDy bit is also set.
	PWR_PUCRA_PU6 = 1UL << 6,   // Port A pull-up bit y (y=0..13) When set, this bit activates the pull-up on PA[y] when APC bit is set in
	                            // PWR_CR3 register. The pull-up is not activated if the corresponding PDy bit is also set.
	PWR_PUCRA_PU5 = 1UL << 5,   // Port A pull-up bit y (y=0..13) When set, this bit activates the pull-up on PA[y] when APC bit is set in
	                            // PWR_CR3 register. The pull-up is not activated if the corresponding PDy bit is also set.
	PWR_PUCRA_PU4 = 1UL << 4,   // Port A pull-up bit y (y=0..13) When set, this bit activates the pull-up on PA[y] when APC bit is set in
	                            // PWR_CR3 register. The pull-up is not activated if the corresponding PDy bit is also set.
	PWR_PUCRA_PU3 = 1UL << 3,   // Port A pull-up bit y (y=0..13) When set, this bit activates the pull-up on PA[y] when APC bit is set in
	                            // PWR_CR3 register. The pull-up is not activated if the corresponding PDy bit is also set.
	PWR_PUCRA_PU2 = 1UL << 2,   // Port A pull-up bit y (y=0..13) When set, this bit activates the pull-up on PA[y] when APC bit is set in
	                            // PWR_CR3 register. The pull-up is not activated if the corresponding PDy bit is also set.
	PWR_PUCRA_PU1 = 1UL << 1,   // Port A pull-up bit y (y=0..13) When set, this bit activates the pull-up on PA[y] when APC bit is set in
	                            // PWR_CR3 register. The pull-up is not activated if the corresponding PDy bit is also set.
	PWR_PUCRA_PU0 = 1UL << 0,   // Port A pull-up bit y (y=0..13) When set, this bit activates the pull-up on PA[y] when APC bit is set in
	                            // PWR_CR3 register. The pull-up is not activated if the corresponding PDy bit is also set.
};
...
enum {
	PWR_PDCRA_PD14 =
	    1UL << 14, // Port A pull-down bit 14 When set, this bit activates the pull-down on PA[14] when APC bit is set in PWR_CR3 register.
	PWR_PDCRA_PD12 = 1UL << 12, // Port A pull-down bit y (y=0..12) When set, this bit activates the pull-down on PA[y] when APC bit is set
	                            // in PWR_CR3 register.
	PWR_PDCRA_PD11 = 1UL << 11, // Port A pull-down bit y (y=0..12) When set, this bit activates the pull-down on PA[y] when APC bit is set
	                            // in PWR_CR3 register.
	PWR_PDCRA_PD10 = 1UL << 10, // Port A pull-down bit y (y=0..12) When set, this bit activates the pull-down on PA[y] when APC bit is set
	                            // in PWR_CR3 register.
	PWR_PDCRA_PD9 = 1UL << 9, // Port A pull-down bit y (y=0..12) When set, this bit activates the pull-down on PA[y] when APC bit is set in
	                          // PWR_CR3 register.
	PWR_PDCRA_PD8 = 1UL << 8, // Port A pull-down bit y (y=0..12) When set, this bit activates the pull-down on PA[y] when APC bit is set in
	                          // PWR_CR3 register.
	PWR_PDCRA_PD7 = 1UL << 7, // Port A pull-down bit y (y=0..12) When set, this bit activates the pull-down on PA[y] when APC bit is set in
	                          // PWR_CR3 register.
	PWR_PDCRA_PD6 = 1UL << 6, // Port A pull-down bit y (y=0..12) When set, this bit activates the pull-down on PA[y] when APC bit is set in
	                          // PWR_CR3 register.
	PWR_PDCRA_PD5 = 1UL << 5, // Port A pull-down bit y (y=0..12) When set, this bit activates the pull-down on PA[y] when APC bit is set in
	                          // PWR_CR3 register.
	PWR_PDCRA_PD4 = 1UL << 4, // Port A pull-down bit y (y=0..12) When set, this bit activates the pull-down on PA[y] when APC bit is set in
	                          // PWR_CR3 register.
	PWR_PDCRA_PD3 = 1UL << 3, // Port A pull-down bit y (y=0..12) When set, this bit activates the pull-down on PA[y] when APC bit is set in
	                          // PWR_CR3 register.
	PWR_PDCRA_PD2 = 1UL << 2, // Port A pull-down bit y (y=0..12) When set, this bit activates the pull-down on PA[y] when APC bit is set in
	                          // PWR_CR3 register.
	PWR_PDCRA_PD1 = 1UL << 1, // Port A pull-down bit y (y=0..12) When set, this bit activates the pull-down on PA[y] when APC bit is set in
	                          // PWR_CR3 register.
	PWR_PDCRA_PD0 = 1UL << 0, // Port A pull-down bit y (y=0..12) When set, this bit activates the pull-down on PA[y] when APC bit is set in
	                          // PWR_CR3 register.
};

// PWR->PDCRB Power Port B pull-down control register
enum {
	PWR_PDCRB_PD15 = 1UL << 15, // Port B pull-down bit y (y=5..15) When set, this bit activates the pull-down on PB[y] when APC bit is set
	                            // in PWR_CR3 register.
	PWR_PDCRB_PD14 = 1UL << 14, // Port B pull-down bit y (y=5..15) When set, this bit activates the pull-down on PB[y] when APC bit is set
	                            // in PWR_CR3 register.
	PWR_PDCRB_PD13 = 1UL << 13, // Port B pull-down bit y (y=5..15) When set, this bit activates the pull-down on PB[y] when APC bit is set
	                            // in PWR_CR3 register.
	PWR_PDCRB_PD12 = 1UL << 12, // Port B pull-down bit y (y=5..15) When set, this bit activates the pull-down on PB[y] when APC bit is set
	                            // in PWR_CR3 register.
	PWR_PDCRB_PD11 = 1UL << 11, // Port B pull-down bit y (y=5..15) When set, this bit activates the pull-down on PB[y] when APC bit is set
	                            // in PWR_CR3 register.
	PWR_PDCRB_PD10 = 1UL << 10, // Port B pull-down bit y (y=5..15) When set, this bit activates the pull-down on PB[y] when APC bit is set
	                            // in PWR_CR3 register.
	PWR_PDCRB_PD9 = 1UL << 9, // Port B pull-down bit y (y=5..15) When set, this bit activates the pull-down on PB[y] when APC bit is set in
	                          // PWR_CR3 register.
	PWR_PDCRB_PD8 = 1UL << 8, // Port B pull-down bit y (y=5..15) When set, this bit activates the pull-down on PB[y] when APC bit is set in
	                          // PWR_CR3 register.
	PWR_PDCRB_PD7 = 1UL << 7, // Port B pull-down bit y (y=5..15) When set, this bit activates the pull-down on PB[y] when APC bit is set in
	                          // PWR_CR3 register.
	PWR_PDCRB_PD6 = 1UL << 6, // Port B pull-down bit y (y=5..15) When set, this bit activates the pull-down on PB[y] when APC bit is set in
	                          // PWR_CR3 register.
	PWR_PDCRB_PD5 = 1UL << 5, // Port B pull-down bit y (y=5..15) When set, this bit activates the pull-down on PB[y] when APC bit is set in
	                          // PWR_CR3 register.
	PWR_PDCRB_PD3 = 1UL << 3, // Port B pull-down bit y (y=0..3) When set, this bit activates the pull-down on PB[y] when APC bit is set in
	                          // PWR_CR3 register.
	PWR_PDCRB_PD2 = 1UL << 2, // Port B pull-down bit y (y=0..3) When set, this bit activates the pull-down on PB[y] when APC bit is set in
	                          // PWR_CR3 register.
	PWR_PDCRB_PD1 = 1UL << 1, // Port B pull-down bit y (y=0..3) When set, this bit activates the pull-down on PB[y] when APC bit is set in
	                          // PWR_CR3 register.
	PWR_PDCRB_PD0 = 1UL << 0, // Port B pull-down bit y (y=0..3) When set, this bit activates the pull-down on PB[y] when APC bit is set in
	                          // PWR_CR3 register.
};
...
enum {
	PWR_PUCRG_PUX =
	    ((1UL << 11) - 1) << 0, // Merged Port G pull-up bit y (y=0..10) When set, this bit activates the pull-up on PG[y] when APC bit is
...
enum {
	PWR_PDCRG_PDX = ((1UL << 11) - 1) << 0, // Merged Port G pull-down bit y (y=0..10) When set, this bit activates the pull-down on PG[y]
...
enum {
	PWR_CR5_R1MODE = 1UL << 8, // Main regular range 1 mode This bit is only valid for the main regulator in range 1 and has no effect on
...
enum {
	...
	RCC_CR_PLLON = 1UL << 24, // Main PLL enable Set and cleared by software to enable the main PLL. Cleared by hardware when entering Stop,
	                          // Standby or Shutdown mode. This bit cannot be reset if the PLL clock is used as the system clock.
	RCC_CR_CSSON = 1UL << 19, // Clock security system enable Set by software to enable the clock security system. When CSSON is set, the
	                          // clock detector is enabled by hardware when the HSE oscillator is ready, and disabled by hardware if a HSE
	                          // clock failure is detected. This bit is set only and is cleared by reset.
	RCC_CR_HSEBYP = 1UL << 18, // HSE crystal oscillator bypass Set and cleared by software to bypass the oscillator with an external clock.
	                           // The external clock must be enabled with the HSEON bit set, to be used by the device. The HSEBYP bit can be
	                           // written only if the HSE oscillator is disabled.
	RCC_CR_HSERDY = 1UL << 17, // HSE clock ready flag Set by hardware to indicate that the HSE oscillator is stable. Note: Once the HSEON
	                           // bit is cleared, HSERDY goes low after 6 HSE clock cycles.
	RCC_CR_HSEON =
	    1UL
	    << 16, // HSE clock enable Set and cleared by software. Cleared by hardware to stop the HSE oscillator when entering Stop, Standby
	           // or Shutdown mode. This bit cannot be reset if the HSE oscillator is used directly or indirectly as the system clock.
	RCC_CR_HSIRDY =
	    1UL << 10, // HSI16 clock ready flag Set by hardware to indicate that HSI16 oscillator is stable. This bit is set only when HSI16 is
	               // enabled by software by setting HSION. Note: Once the HSION bit is cleared, HSIRDY goes low after 6 HSI16 clock cycles.
	RCC_CR_HSIKERON = 1UL << 9, // HSI16 always enable for peripheral kernels. Set and cleared by software to force HSI16 ON even in Stop
	                            // modes. The HSI16 can only feed USARTs and I<sup>2</sup>Cs peripherals configured with HSI16 as kernel
	                            // clock. Keeping the HSI16 ON in Stop mode allows to avoid slowing down the communication speed because of
	...
	RCC_CR_HSION = 1UL << 8,    // HSI16 clock enable Set and cleared by software. Cleared by hardware to stop the HSI16 oscillator when
	                         // entering Stop, Standby or Shutdown mode. Set by hardware to force the HSI16 oscillator ON when STOPWUCK=1 or
	                         // HSIASFS = 1 when leaving Stop modes, or in case of failure of the HSE crystal oscillator. This bit is set by
	                         // hardware if the HSI16 is used directly or indirectly as system clock.
};
...
enum {
	RCC_ICSCR_HSITRIM =
	    ((1UL << 7) - 1)
	    << 24, // HSI16 clock trimming These bits provide an additional user-programmable trimming value that is added to the HSICAL[7:0]
	           // bits. It can be programmed to adjust to variations in voltage and temperature that influence the frequency of the HSI16.
	           // The default value is 16, which, when added to the HSICAL value, should trim the HSI16 to 16 MHz � 1 %.
	RCC_ICSCR_HSICAL = ((1UL << 8) - 1)
...
enum {
	RCC_CFGR_MCOPRE =
	    ((1UL << 3) - 1) << 28, // Microcontroller clock output prescaler These bits are set and cleared by software. It is highly
	                            // recommended to change this prescaler before MCO output is enabled. Others: not allowed
	RCC_CFGR_MCOSEL = ((1UL << 4) - 1) << 24, // Microcontroller clock output Set and cleared by software. Others: Reserved Note: This clock
	                                          // output may have some truncated cycles at startup or during MCO clock source switching.
	RCC_CFGR_PPRE2 =
	    ((1UL << 3) - 1)
	    << 11, // APB2 prescaler Set and cleared by software to control the division factor of the APB2 clock (PCLK2). 0xx: HCLK not divided
	RCC_CFGR_PPRE1 =
	    ((1UL << 3) - 1)
	    << 8, // APB1 prescaler Set and cleared by software to control the division factor of the APB1 clock (PCLK1). 0xx: HCLK not divided
	RCC_CFGR_HPRE = ((1UL << 4) - 1)
	                << 4, // AHB prescaler Set and cleared by software to control the division factor of the AHB clock. Note: Depending on
	                      // the device voltage range, the software has to set correctly these bits to ensure that the system frequency does
	                      // not exceed the maximum allowed frequency (for more details please refer to Section 6.1.5: Dynamic voltage
	...
	RCC_CFGR_SWS = ((1UL << 2) - 1)
	               << 2, // System clock switch status Set and cleared by hardware to indicate which clock source is used as system clock.
	RCC_CFGR_SW = ((1UL << 2) - 1)
	              << 0, // System clock switch Set and cleared by software to select system clock source (SYSCLK). Configured by hardware to
	                    // force HSI16 oscillator selection when exiting stop and standby modes or in case of failure of the HSE oscillator.
};
...
enum {
	RCC_PLLCFGR_PLLPDIV = ((1UL << 5) - 1) << 27, // Main PLLP division factor Set and cleared by software to control the PLL “P” frequency.
	                                              // PLL “P” output clock frequency = VCO frequency / PLLPDIV. ....
	RCC_PLLCFGR_PLLR =
	    ((1UL << 2) - 1) << 25, // Main PLL division factor for PLL “R” clock (system clock) Set and cleared by software to control the
	                            // frequency of the main PLL output clock PLLCLK. This output can be selected as system clock. These bits
	                            // can be written only if PLL is disabled. PLL “R” output clock frequency = VCO frequency / PLLR with PLLR =
	                            // 2, 4, 6, or 8 Note: The software has to set these bits correctly not to exceed 170 MHz on this domain.
	RCC_PLLCFGR_PLLREN =
	    1UL << 24, // PLL “R” clock output enable Set and reset by software to enable the PLL “R” clock output of the PLL (used as system
	               // clock). This bit cannot be written when PLL “R” clock output of the PLL is used as System Clock. In order to save
	               // power, when the PLL “R” clock output of the PLL is not used, the value of PLLREN should be 0.
	RCC_PLLCFGR_PLLQ =
	    ((1UL << 2) - 1)
	    << 21, // Main PLL division factor for PLL “Q” clock. Set and cleared by software to control the frequency of the main PLL output
	           // clock PLL “Q” clock. This output can be selected for USB, RNG, SAI (48 MHz clock). These bits can be written only if PLL
	           // is disabled. PLL “Q” output clock frequency = VCO frequency / PLLQ with PLLQ = 2, 4, 6, or 8 Note: The software has to set
	           // these bits correctly not to exceed 170 MHz on this domain.
	RCC_PLLCFGR_PLLQEN =
	    1UL << 20, // Main PLL “Q” clock output enable Set and reset by software to enable the PLL “Q” clock output of the PLL. In order to
	               // save power, when the PLL “Q” clock output of the PLL is not used, the value of PLLQEN should be 0.
	RCC_PLLCFGR_PLLP = 1UL << 17, // Main PLL division factor for PLL “P” clock. Set and cleared by software to control the frequency of the
	...
	RCC_PLLCFGR_PLLPEN =
	    1UL << 16, // Main PLL PLL “P” clock output enable Set and reset by software to enable the PLL “P” clock output of the PLL. In order
	               // to save power, when the PLL “P” clock output of the PLL is not used, the value of PLLPEN should be 0.
	RCC_PLLCFGR_PLLN =
	    ((1UL << 7) - 1) << 8, // Main PLL multiplication factor for VCO Set and cleared by software to control the multiplication factor of
	                           // the VCO. These bits can be written only when the PLL is disabled. VCO output frequency = VCO input
	                           // frequency x PLLN with 8 =< PLLN =< 127 ... ... Note: The software has to set correctly these bits to
	                           // assure that the VCO output frequency is within the range defined in the device datasheet.
	RCC_PLLCFGR_PLLM =
	    ((1UL << 4) - 1) << 4, // Division factor for the main PLL input clock Set and cleared by software to divide the PLL input clock
	                           // before the VCO. These bits can be written only when all PLLs are disabled. VCO input frequency = PLL input
	                           // clock frequency / PLLM with 1 <= PLLM <= 16 ... Note: The software has to set these bits correctly to
	                           // ensure that the VCO input frequency is within the range defined in the device datasheet.
	RCC_PLLCFGR_PLLSRC =
	    ((1UL << 2) - 1)
	    << 0, // Main PLL entry clock source Set and cleared by software to select PLL clock source. These bits can be written only when PLL
	          // is disabled. In order to save power, when no PLL is used, the value of PLLSRC should be 00.
};
...
enum {
	RCC_CIER_HSI48RDYIE = 1UL << 10, // HSI48 ready interrupt enable Set and cleared by software to enable/disable interrupt caused by the
	                                 // internal HSI48 oscillator.
	RCC_CIER_LSECSSIE = 1UL << 9,    // LSE clock security system interrupt enable Set and cleared by software to enable/disable interrupt
	                                 // caused by the clock security system on LSE.
	...
	RCC_CIER_HSERDYIE = 1UL << 4, // HSE ready interrupt enable Set and cleared by software to enable/disable interrupt caused by the HSE
	                              // oscillator stabilization.
	RCC_CIER_HSIRDYIE = 1UL << 3, // HSI16 ready interrupt enable Set and cleared by software to enable/disable interrupt caused by the
	                              // HSI16 oscillator stabilization.
	RCC_CIER_LSERDYIE = 1UL << 1, // LSE ready interrupt enable Set and cleared by software to enable/disable interrupt caused by the LSE
	                              // oscillator stabilization.
	RCC_CIER_LSIRDYIE = 1UL << 0, // LSI ready interrupt enable Set and cleared by software to enable/disable interrupt caused by the LSI
	                              // oscillator stabilization.
};
...
enum {
	RCC_CIFR_HSI48RDYF =
	    1UL << 10, // HSI48 ready interrupt flag Set by hardware when the HSI48 clock becomes stable and HSI48RDYIE is set in a response to
	               // setting the HSI48ON (refer to Clock recovery RC register (RCC_CRRCR)). Cleared by software setting the HSI48RDYC bit.
	...
	RCC_CIFR_CSSF = 1UL << 8,    // Clock security system interrupt flag Set by hardware when a failure is detected in the HSE oscillator.
	                             // Cleared by software setting the CSSC bit.
	RCC_CIFR_PLLRDYF = 1UL << 5, // PLL ready interrupt flag Set by hardware when the PLL locks and PLLRDYDIE is set. Cleared by software
	                             // setting the PLLRDYC bit.
	RCC_CIFR_HSERDYF = 1UL << 4, // HSE ready interrupt flag Set by hardware when the HSE clock becomes stable and HSERDYDIE is set. Cleared
	                             // by software setting the HSERDYC bit.
	RCC_CIFR_HSIRDYF = 1UL << 3, // HSI16 ready interrupt flag Set by hardware when the HSI16 clock becomes stable and HSIRDYDIE is set in a
	                             // response to setting the HSION (refer to Clock control register (RCC_CR)). When HSION is not set but the
	                             // HSI16 oscillator is enabled by the peripheral through a clock request, this bit is not set and no
	                             // interrupt is generated. Cleared by software setting the HSIRDYC bit.
	RCC_CIFR_LSERDYF = 1UL << 1, // LSE ready interrupt flag Set by hardware when the LSE clock becomes stable and LSERDYDIE is set. Cleared
	                             // by software setting the LSERDYC bit.
	RCC_CIFR_LSIRDYF = 1UL << 0, // LSI ready interrupt flag Set by hardware when the LSI clock becomes stable and LSIRDYDIE is set. Cleared
	                             // by software setting the LSIRDYC bit.
};
...
enum {
	...
	RCC_AHB1RSTR_FLASHRST = 1UL << 8,   // Flash memory interface reset Set and cleared by software. This bit can be activated only when the
...
enum {
	...
	RCC_AHB1ENR_FLASHEN = 1UL << 8,   // Flash memory interface clock enable Set and cleared by software. This bit can be disabled only when
	                                  // the Flash is in power down mode.
...
enum {
	...
	RCC_APB1ENR1_WWDGEN = 1UL << 11, // Window watchdog clock enable Set by software to enable the window watchdog clock. Reset by hardware
...
enum {
	...
	RCC_APB1SMENR1_WWDGSMEN = 1UL << 11, // Window watchdog clocks enable during Sleep and Stop modes Set and cleared by software. This bit
	                                     // is forced to ‘1’ by hardware when the hardware WWDG option is activated.
...
enum {
	RCC_CCIPR_ADC345SEL = ((1UL << 2) - 1) << 30, // ADC3/4/5 clock source selection These bits are set and cleared by software to select
	                                              // the clock source used by the ADC345 interface.
	RCC_CCIPR_ADC12SEL = ((1UL << 2) - 1) << 28,  // ADC1/2 clock source selection These bits are set and cleared by software to select the
	                                              // clock source used by the ADC interface.
	RCC_CCIPR_CLK48SEL = ((1UL << 2) - 1) << 26,  // 48 MHz clock source selection These bits are set and cleared by software to select the
	                                              // 48 MHz clock source used by USB device FS and RNG.
	RCC_CCIPR_FDCANSEL = ((1UL << 2) - 1) << 24,  // None
	RCC_CCIPR_I2S23SEL =
	    ((1UL << 2) - 1) << 22, // clock source selection These bits are set and cleared by software to select the I2S23 clock source.
	RCC_CCIPR_SAI1SEL =
	    ((1UL << 2) - 1) << 20, // clock source selection These bits are set and cleared by software to select the SAI clock source.
	RCC_CCIPR_LPTIM1SEL =
	    ((1UL << 2) - 1)
	    << 18, // Low power timer 1 clock source selection These bits are set and cleared by software to select the LPTIM1 clock source.
	RCC_CCIPR_I2C3SEL =
	    ((1UL << 2) - 1) << 16, // I2C3 clock source selection These bits are set and cleared by software to select the I2C3 clock source.
	RCC_CCIPR_I2C2SEL =
	    ((1UL << 2) - 1) << 14, // I2C2 clock source selection These bits are set and cleared by software to select the I2C2 clock source.
	RCC_CCIPR_I2C1SEL =
	    ((1UL << 2) - 1) << 12, // I2C1 clock source selection These bits are set and cleared by software to select the I2C1 clock source.
	...
	RCC_CCIPR_UART5SEL =
	    ((1UL << 2) - 1) << 8, // UART5 clock source selection These bits are set and cleared by software to select the UART5 clock source.
	RCC_CCIPR_UART4SEL =
	    ((1UL << 2) - 1) << 6, // UART4 clock source selection This bit is set and cleared by software to select the UART4 clock source.
	RCC_CCIPR_USART3SEL =
	    ((1UL << 2) - 1) << 4, // USART3 clock source selection This bit is set and cleared by software to select the USART3 clock source.
	RCC_CCIPR_USART2SEL =
	    ((1UL << 2) - 1) << 2, // USART2 clock source selection This bit is set and cleared by software to select the USART2 clock source.
	RCC_CCIPR_USART1SEL =
	    ((1UL << 2) - 1) << 0, // USART1 clock source selection This bit is set and cleared by software to select the USART1 clock source.
};
...
enum {
	...
	RCC_BDCR_RTCSEL =
	    ((1UL << 2) - 1) << 8, // RTC clock source selection Set by software to select the clock source for the RTC. Once the RTC clock
	                           // source has been selected, it cannot be changed anymore unless the RTC domain is reset, or unless a failure
	                           // is detected on LSE (LSECSSD is set). The BDRST bit can be used to reset them.
	RCC_BDCR_LSECSSD = 1UL << 6, // CSS on LSE failure Detection Set by hardware to indicate when a failure has been detected by the Clock
	                             // Security System on the external 32 kHz oscillator (LSE).
	RCC_BDCR_LSECSSON =
	    1UL << 5, // CSS on LSE enable Set by software to enable the Clock Security System on LSE (32 kHz oscillator). LSECSSON must be
	              // enabled after the LSE oscillator is enabled (LSEON bit enabled) and ready (LSERDY flag set by hardware), and after the
	              // RTCSEL bit is selected. Once enabled this bit cannot be disabled, except after a LSE failure detection (LSECSSD =1). In
	              // that case the software MUST disable the LSECSSON bit.
	RCC_BDCR_LSEDRV = ((1UL << 2) - 1) << 3, // LSE oscillator drive capability Set by software to modulate the LSE oscillator’s drive
	                                         // capability. The oscillator is in Xtal mode when it is not in bypass mode.
	RCC_BDCR_LSEBYP = 1UL << 2, // LSE oscillator bypass Set and cleared by software to bypass oscillator in debug mode. This bit can be
	                            // written only when the external 32 kHz oscillator is disabled (LSEON=0 and LSERDY=0).
	RCC_BDCR_LSERDY =
	    1UL << 1, // LSE oscillator ready Set and cleared by hardware to indicate when the external 32 kHz oscillator is stable. After the
...
enum {
	RCC_CSR_LPWRRSTF = 1UL << 31, // Low-power reset flag Set by hardware when a reset occurs due to illegal Stop, Standby or Shutdown mode
	                              // entry. Cleared by writing to the RMVF bit.
	RCC_CSR_WWDGRSTF =
	    1UL << 30, // Window watchdog reset flag Set by hardware when a window watchdog reset occurs. Cleared by writing to the RMVF bit.
	RCC_CSR_IWDGRSTF = 1UL << 29, // Independent window watchdog reset flag Set by hardware when an independent watchdog reset domain
	...
	RCC_CSR_OBLRSTF = 1UL << 25, // Option byte loader reset flag Set by hardware when a reset from the Option Byte loading occurs. Cleared
	                             // by writing to the RMVF bit.
	RCC_CSR_RMVF = 1UL << 23,    // Remove reset flag Set by software to clear the reset flags.
	RCC_CSR_LSIRDY =
	    1UL << 1, // LSI oscillator ready Set and cleared by hardware to indicate when the LSI oscillator is stable. After the LSION bit is
	              // cleared, LSIRDY goes low after 3 LSI oscillator clock cycles. This bit can be set even if LSION = 0 if the LSI is
	              // requested by the Clock Security System on LSE, by the Independent Watchdog or by the RTC.
	RCC_CSR_LSION = 1UL << 0, // LSI oscillator enable Set and cleared by software.
};

// RCC->CRRCR Clock recovery RC register
enum {
	RCC_CRRCR_HSI48CAL = ((1UL << 9) - 1) << 7, // HSI48 clock calibration These bits are initialized at startup with the factory-programmed
	                                            // HSI48 calibration trim value. They are ready only.
	RCC_CRRCR_HSI48RDY = 1UL << 1, // HSI48 clock ready flag Set by hardware to indicate that HSI48 oscillator is stable. This bit is set
	                               // only when HSI48 is enabled by software by setting HSI48ON.
	RCC_CRRCR_HSI48ON = 1UL << 0,  // HSI48 clock enable Set and cleared by software. Cleared by hardware to stop the HSI48 when entering in
	                               // Stop, Standby or Shutdown modes.
};
...
enum {
	...
	RCC_CCIPR2_I2C4SEL =
	    ((1UL << 2) - 1) << 0, // I2C4 clock source selection These bits are set and cleared by software to select the I2C4 clock source.
};
