...
enum {
	   LED0_PIN      = PA5,      // 64 pin nucleo 
       LED1_PIN      = PB8,      // 32 pin mini nucleo
};
...
static inline void led1_off(void) { digitalLo(LED1_PIN); }
static inline void led1_on(void) { digitalHi(LED1_PIN); }
static inline void led1_toggle(void) { digitalToggle(LED1_PIN); }
