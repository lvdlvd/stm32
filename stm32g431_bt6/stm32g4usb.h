#pragma once
/*
   Low level driver for STM32G4xx USB_FS devices

   See RM0440 STM32G4xx Rev 7 Section 45 p.2003
   https://www.st.com/resource/en/reference_manual/rm0440-stm32g4-series-advanced-armbased-32bit-mcus-stmicroelectronics.pdf

   based on STM32F103 USB.
   ...
   Relevant interrupts:
	   ...
	   USB_HP_IRQn  = 19, // USB High Priority interrupts -> double-buffered BULK and ISO CTR
	   USB_LP_IRQn = 20, // USB Low Priority interrupts -> all ISTR interupts
	   USBWakeUP_IRQn = 42, // USB Device FS Wakeup through EXTI line 18 interrupt

   The register mapping generated from the SVD is far less convenient to work with so we manually redid this device.
*/
...
extern struct USB_Type {
	...
	volatile uint32_t LPMCSR;  // @54 LPM control and status register
	volatile uint32_t BCDR;	   // @58 Battery charging detector
} USB;						   // @0x40005C00
...
inline void usb_ep_set_stat_rx(uint8_t ep, uint16_t val) {
	USB.EPR[ep & 0x7f] = USB_EP_CTR | ((USB.EPR[ep & 0x7f] & (USB_EP_CFG | USB_EPRx_STAT_RX)) ^ ((val << 12) & USB_EPRx_STAT_RX));
}
inline void usb_ep_set_stat_tx(uint8_t ep, uint16_t val) {
	USB.EPR[ep & 0x7f] = USB_EP_CTR | ((USB.EPR[ep & 0x7f] & (USB_EP_CFG | USB_EPRx_STAT_TX)) ^ ((val << 4) & USB_EPRx_STAT_TX));
}
...
inline void usb_ep_clr_dtog_rx(uint8_t ep) {
	USB.EPR[ep & 0x7f] = USB_EP_CTR | (USB.EPR[ep & 0x7f] & (USB_EP_CFG | USB_EPRx_DTOG_RX));
}
inline void usb_ep_set_dtog_rx(uint8_t ep) {
	USB.EPR[ep & 0x7f] = USB_EP_CTR | ((USB.EPR[ep & 0x7f] & (USB_EP_CFG | USB_EPRx_DTOG_RX)) ^ USB_EPRx_DTOG_RX);
}
inline void usb_ep_clr_dtog_tx(uint8_t ep) {
	USB.EPR[ep & 0x7f] = USB_EP_CTR | (USB.EPR[ep & 0x7f] & (USB_EP_CFG | USB_EPRx_DTOG_TX));
}
...
enum {
	...
	// DIR bit=0, CTR_TX bit is set in the USB_EPnR register related to the interrupting endpoint. The interrupting transaction is
	// of IN type (data transmitted by the USB peripheral to the host PC). DIR bit=1, CTR_RX bit or both CTR_TX/CTR_RX are set in
	// the USB_EPnR register related to the interrupting endpoint. The interrupting transaction is of OUT type (data received by the
...
extern union {
	...
	volatile uint16_t buf[512];	// only accessible as uint16 or uint8
} USB_PMA;				// @ 0x40006000
