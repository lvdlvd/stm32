...
// only works for the positive (non-core) irqns
...
static inline void NVIC_SetPendingIRQ(enum IRQn_Type IRQn) { (&NVIC.ISPR0)[IRQn >> 5] = 1U << (IRQn & 0x1F); }
static inline void NVIC_ClearPendingIRQ(enum IRQn_Type IRQn) { (&NVIC.ICPR0)[IRQn >> 5] = 1U << (IRQn & 0x1F); }
