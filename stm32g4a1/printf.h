...
static inline uint16_t ringbuffer_avail(const struct Ringbuffer *rb) { return rb->head - rb->tail; }						  // 0..size -1
static inline uint16_t ringbuffer_free(const struct Ringbuffer *rb) { return sizeof(rb->buf) - (rb->head - rb->tail) - 1; }  // size-1 .. 0
static inline int		ringbuffer_empty(const struct Ringbuffer *rb) { return rb->head == rb->tail; }
static inline int		ringbuffer_full(const struct Ringbuffer *rb) { return sizeof(rb->buf) - 3 < (uint16_t)(rb->head - rb->tail); }
static inline void		ringbuffer_clear(struct Ringbuffer *rb) { rb->head = rb->tail; }  // may race with get_tail
static inline void		ringbuffer_put_head(struct Ringbuffer *rb, uint8_t c) { rb->buf[rb->head++ & (sizeof(rb->buf) - 1)] = c; }
static inline uint8_t	ringbuffer_get_tail(struct Ringbuffer *rb) { return rb->buf[rb->tail++ & (sizeof(rb->buf) - 1)]; }
